...
void menuGeneralSetup(uint8_t event)
{
  ...
  MENU(STR_MENURADIOSETUP
	  ...
	  , LABEL(ALARMS)
	  , 0
	  , CASE_PCBSKY9X(0) CASE_PCBSKY9X(0) 0
	  , 0										// ITEM_SETUP_BACKLIGHT_MODE
	  , 0
	  , IF_ROTARY_ENCODERS(0) LABEL(BACKLIGHT)
	  , 0
	  , 0
	  , CASE_CPUARM(0) CASE_REVPLUS(0) CASE_PWM_BACKLIGHT(0) CASE_PWM_BACKLIGHT(0) 0
	  , CASE_SPLASH_PARAM(0) CASE_GPS(0) CASE_GPS(0) CASE_PXX(0) CASE_CPUARM(0) CASE_CPUARM(0) IF_FAI_CHOICE(0) CASE_MAVLINK(0) CASE_CPUARM(0) 0
	  , COL_TX_MODE, CASE_PCBTARANIS(0) 1/*to force edit mode*/});
